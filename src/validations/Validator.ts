...
const validate = (obj: any, key: string) => (
    tuple: ValidatorTuple
): ValidateObject => {
...
export function Validator<T>(
    ...
    const validated: ValidatorCheck = Object.keys(values).reduce(
        (acc: any, key: string) => {
            const errors = specs[key]
                ...
                .filter(obj =>
                    unpack(options, "showSuccess").value ? true : !obj.valid
                );
    ...
    if (flatMessages.value) {
        const flatted = Object.keys(validated).map(x =>
            validated[x].map(y => y.message)
        );
